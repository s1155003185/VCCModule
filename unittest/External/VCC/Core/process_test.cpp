...
TEST(ProcessTest, Normal)
{
    ...
    EXPECT_TRUE(HasPrefix(ProcessService::Execute(logProperty, L"", L"", L"git", L"git --version"), L"git version"));
}
...
TEST(ProcessTest, Exception)
{
    ...
    bool isError = false;
    try {
        ProcessService::Execute(logProperty, L"", L"", L"git", L"git -version");
    } catch (exception &e) {
